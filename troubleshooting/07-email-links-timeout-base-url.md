...
## Root Cause
`BASE_URL` was set to the Zabbix server's internal lab address, which is
only reachable from inside the lab's internal network — not from wherever
the email link was actually being opened.
...
network segment as Zabbix once the project moves to production, so
`BASE_URL` in `.env` can point there and the email links resolve correctly
from any client that can reach that network.
